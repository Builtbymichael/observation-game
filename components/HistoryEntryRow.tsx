  ...
  return (
    <div className="border border-secondary p-4 rounded-lg animate-fade-in space-y-3">
      <div className="flex justify-between items-start gap-4">
        <p className="font-serif text-foreground flex-1 text-left">{game.question}</p>
        <span
          className={`text-xs uppercase font-bold px-2 py-1 rounded-full text-white whitespace-nowrap flex-shrink-0 ${isCorrect ? "bg-correct" : "bg-incorrect"}`}
        >
      ...
      <div className="text-xs text-muted-foreground pt-2 border-t border-secondary/50">
        <p>
          Answered: {formatDate(game.answeredDate)} ({game.delayDays}-day recall)
        </p>
      </div>
    </div>
  )
